...
def generate_trajectory_coeffs(
...
def create_three_phase_trajectory(
    positions: List[float],
    velocities: List[float],
    accelerations: List[float],
    time: npt.ArrayLike,
    ...
    for ts in time:
        if ts < t0 + 2 * delta:
            current_val = (
                coeffs_phase1[0]
                + coeffs_phase1[1] * ts
                + coeffs_phase1[2] * pow(ts, 2)
                + coeffs_phase1[3] * pow(ts, 3)
                + coeffs_phase1[4] * pow(ts, 4)
                + coeffs_phase1[5] * pow(ts, 5)
            )
        elif ts < tf - 2 * delta:
            ...
            current_val = (
                coeffs_phase3[0]
                + coeffs_phase3[1] * ts
                + coeffs_phase3[2] * pow(ts, 2)
                + coeffs_phase3[3] * pow(ts, 3)
                + coeffs_phase3[4] * pow(ts, 4)
                + coeffs_phase3[5] * pow(ts, 5)
            )
    ...
    for ts in time:
        if ts < t0 + 2 * delta:
            current_val = (
                coeffs_phase1[1]
                + 2 * coeffs_phase1[2] * ts
                + 3 * coeffs_phase1[3] * pow(ts, 2)
                + 4 * coeffs_phase1[4] * pow(ts, 3)
                + 5 * coeffs_phase1[5] * pow(ts, 4)
            )
        elif ts < tf - 2 * delta:
            ...
            current_val = (
                coeffs_phase3[1]
                + 2 * coeffs_phase3[2] * ts
                + 3 * coeffs_phase3[3] * pow(ts, 2)
                + 4 * coeffs_phase3[4] * pow(ts, 3)
                + 5 * coeffs_phase3[5] * pow(ts, 4)
            )
        velocity_trajectory.append(current_val)

    trajectories = {"position": position_trajectory, "velocity": velocity_trajectory}
    return trajectories
